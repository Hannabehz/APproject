...
public class CartViewController {
    ...
    public void initialize() {
        ...
        categoriesColumn.setCellValueFactory(cellData -> new SimpleStringProperty(cellData.getValue().getCategories()));
        ...
        actionColumn.setCellFactory(param -> new TableCell<>() {
...

            {
                removeButton.setOnAction(event -> {
                    ...
                    cartService.removeFromCart(token, item.getItemId())
    ...
    private void submitOrder() {
        ...
        String vendorId = cartTable.getItems().get(0).getVendorId();
        if (vendorId == null || vendorId.trim().isEmpty()) {
            showAlert(Alert.AlertType.ERROR, "خطا", "شناسه رستوران معتبر نیست!");
            return;
        }

        boolean allSameVendor = cartTable.getItems().stream()
                .allMatch(item -> item.getVendorId().equals(vendorId));
        if (!allSameVendor) {
