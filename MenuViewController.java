...
public class MenuViewController {
            ...
            for (String title : menuTitles) {
                ...
                if (items != null) {
                    for (FoodDTO item : items) {
                        ...
                        Label vendorIdLabel = new Label("شناسه فروشنده: " + item.getVendorId());
                        ...
                        addButton.setOnAction(event -> addToCart(item.getItemId().toString(), item.getSupply(), quantityLabel));

                        Button removeButton = new Button("-");
                        removeButton.setStyle("-fx-font-size: 12px; -fx-background-color: #F44336; -fx-text-fill: white;");
                        removeButton.setOnAction(event -> removeFromCart(item.getItemId().toString(), quantityLabel));
    ...
    private void addToCart(String itemId, int supply, Label quantityLabel) {
        ...
        cartService.addToCart(token, itemId, 1).thenAcceptAsync(response -> Platform.runLater(() -> {
    ...
    private void removeFromCart(String itemId, Label quantityLabel) {
        int currentQuantity = Integer.parseInt(quantityLabel.getText());
        if (currentQuantity <= 0) {
            return;
        }

        cartService.removeFromCart(token, itemId).thenAcceptAsync(response -> Platform.runLater(() -> {
            if (response == null || !response.containsKey("status")) {
                showAlert(Alert.AlertType.ERROR, "خطا", "پاسخ سرور نامعتبر است");
                return;
            }
    ...
    private void goBack() {
        try {
            ...
            showAlert(Alert.AlertType.ERROR, "خطا", "خطا در بازگشت به صفحه اصلی: " + e.getMessage());
        }
    }
